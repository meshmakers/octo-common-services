...
internal class XmlDocOperationTransformer(IOptions<OctoOpenApiOptions> options) : IOpenApiDocumentTransformer
{
            ...
            foreach (var descriptionGroup in context.DescriptionGroups)
            {
                foreach (var apiDescription in descriptionGroup.Items)
                {
                    ...
                    if (apiDescription.ActionDescriptor is ControllerActionDescriptor controllerActionDescriptor)
                    {
                        var xmlDocNodes =
                            xmlDoc.SelectNodes(
                                $"//member[starts-with(@name, 'M:{controllerActionDescriptor.MethodInfo.DeclaringType?.FullName}.{controllerActionDescriptor.MethodInfo.Name}')]");
                        if (xmlDocNodes == null)
                        {
                            return Task.CompletedTask;
                        }
                        ...
                        foreach (XmlNode node in xmlDocNodes)
                        {
                            ...
                            {
                                var operationType = HttpMethod.Parse(apiDescription.HttpMethod);
                                ...
                                SetOperationSummary(node, operation.Value);

                                if (operation.Value.Value.Parameters != null)
                                {
                                    foreach (var openApiParameter in operation.Value.Value.Parameters)
                                    {
                                        var parameterNode =
                                            node.SelectSingleNode($"param[@name='{openApiParameter.Name}']");
    ...
    private static void SetOperationSummary(XmlNode node, KeyValuePair<HttpMethod, OpenApiOperation> operation)
    {
