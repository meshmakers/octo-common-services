...
| **2 — Reconciler + lease** | Replace `FailedTenantRetryBackgroundService` internals with the Mongo-backed reconciler + lease; resume `Creating`, finish `Deleting`. | Restart-loss + multi-pod race + silent drop (defects 2, 3) | Med |
