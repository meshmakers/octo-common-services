...
| **3 — Delete/Create serialization** — ✅ **Implemented 2026-07-11** (`dev/4348-tenant-provisioning-phase0`) | `TenantsController.Delete` writes a `Deleting` tombstone before the two-phase drop and removes it once the drop completes (and best-effort on failure, so a failed delete never permanently blocks re-create). Per Q2 it refuses (`409`) to delete a tenant still `Creating`. `TenantsController.Post` consults the store first and returns `409 "deletion still in progress, retry later"` when the target is `Deleting` — the precise signal that closes defect 1's race. `MarkDeletingAsync` is update-only (no phantom tombstone for legacy tenants). | Defect 1 root cause | Med |
| **4 — Operator commands** — ✅ **Implemented 2026-07-11** (`dev/4348-tenant-provisioning-phase0`) | Store gains `RequeueForReconcileAsync` (reset to Creating, zero the attempt budget, clear error + lease). Asset-repo exposes `GET tenants/lifecycle?childTenantId=` and `POST tenants/rerunSetup?childTenantId=` returning a `TenantLifecycleDto` (404 when no record). SDK: `GetTenantLifecycleAsync` / `ReRunTenantSetupAsync`. octo-cli: `GetTenantLifecycle -tid` and `ReRunTenantSetup -tid`. (`CompleteTenantSetup` is the same operation as `ReRunTenantSetup` — one command covers both intents: requeue and let the reconciler finish.) | Operability | Low |
| **5 — Cleanup** — ✅ **Bus retry done 2026-07-11**; in-memory removal **deliberately deferred** (`dev/4348-tenant-provisioning-phase0`) | Added bounded in-process `UseMessageRetry` (1s/5s/15s) to the MassTransit bus so a transient consumer failure self-heals instead of faulting the message on first error (idempotent consumers make this safe). **NOT** removed: `_pendingIdentityDataTenantIds` / `FailedTenantRegistry` / `TenantsInHandling` — see the note below. | Debt / defense-in-depth | Low |

### Why the in-memory paths were NOT removed (Phase 5 re-scope)

The concept originally called for deleting the in-memory sets "in favour of the reconciler". On implementation this proved **unsafe**, because the Phase 2 reconciler is deliberately narrow:

- It runs **only in the store owner (asset-repo)**; the other services (identity, bot, reporting, comms) run their own `SetupTenantAsync` and still rely on `_pendingIdentityDataTenantIds` / `FailedTenantRegistry` for their per-service retry.
- It re-runs **only `CheckSetupIdentityDataAsync`** (identity default-config seeding — the shared, AB#4348-relevant invariant). It does **not** re-drive each service's own CK-model import or `StartTenantAsync`, which `FailedTenantRegistry` still owns.
- `TenantsInHandling` is the **create-consumer concurrency guard**, a different concern from the reconcile lease.

Crucially, defect 2's "silently dropped message → permanent stall" is **already neutralised** by Phase 2: a tenant left `Creating` by a dropped message is driven to completion by the durable reconciler regardless. So the in-memory paths are now a **redundant-but-harmless fast path**, not a correctness risk. Fully removing them (and wiring a durable DLQ) would require a **per-service reconciler** that re-drives each service's full setup — the endpoints are intentionally `Temporary = true` (ephemeral, non-durable), so a durable dead-letter queue is architecturally inconsistent with the current design. That is tracked as potential **future work (Phase 6)**, not forced here.
